class Order < ActiveRecord::Base
      ...
      ActiveRecord::Base.transaction do
        ...
        # 用户首次充值
          if @user.orders.count == 1 and self.status == 0
              ...
              normal_recharge!; true
          end
        end end end end
      end
    rescue =>e
      logger.error("### #{__method__},(#{__FILE__}, #{__LINE__}) recharge failed #{e.to_s} #{type}")
      false # 充值失败
    end
  end
  ...
  def first_recharge!
    # 找到对应的充值规则
    rule = @recharge_list.find{|x| x["id"] == gid and x["name"] == ginfo}
    # 元宝
    ...
    raise "no such goods #{type}" if goods_info.nil?
    # 更新用户道具记录
