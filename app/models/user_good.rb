...
class UserGood < ActiveRecord::Base
  ...
  def self.update_goods(user, goods)
    goods.each do |g|
      ...
      type    = (g[:type] || '').to_s
      ug      = UserGood.find_by_user_id_and_g_type(user.id, type)
      if ug.nil?
        ug = UserGood.new(user_id: user.id, g_type: type)
      end
      ug.number = number
      ug.save
    end
