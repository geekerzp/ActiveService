...
class FriendController < ApplicationController
  ...
  def apply
    ...
    if Relationship.check_relationship(user_id, receiver_id, Relationship::RELATIONSHIP_FRIEND)
      render_result(ResultCode::ALREADY_FRIENDS, {err_msg: 'already friends, do not have to apply.'})
      return
    end

    # 如果之前已经申请过, 不能再申请
    if FriendApplyRecorder.check_apply(user_id, receiver_id)
      render_result(ResultCode::ALREADY_APPLIED, {err_msg: 'already applied.'})
      return
    end
    ...
    if friends_size > friends_limit
      render_result(ResultCode::BEYOND_FRIEND_LIMIT, {err_msg: 'beyond friends number limit.'})
      return
    end
  ...
  def follow
    ...
    if Relationship.check_relationship(user_id, receiver_id, Relationship::RELATIONSHIP_FOLLOW)
      render_result(ResultCode::ALREADY_FOLLOWS, {err_msg: 'already follows, do not have to follow.'})
      return
    end

    # 如果关注上限超过其对应的vip上限，不能再关注
    follow_size = Relationship.where('user_id = ? AND relatioin_type = ?', user_id,
    ...
    if follow_size > follow_limit
      render_result(ResultCode::BEYOND_FOLLOW_LIMIT, {err_msg: 'beyond follow number limit.'})
      return
    end
