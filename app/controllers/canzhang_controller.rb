class CanzhangController < ApplicationController
  #
  # 获取拥有某一残章的用户的列表。
  # 随机选取三位。
  #
  ...
  def update_result
    ...
    if canzhang.nil?
      ...
      render_result(ResultCode::OK, {is_grab: 0})
      return
    end
  ...
  def get_my_canzhangs
    ...
    render_result(ResultCode::OK, canzhangs)
  end
  ...
  def update_canzhangs
    ...
    if canzhangs.nil? || !canzhangs.kind_of?(Array)
      render_result(ResultCode::INVALID_PARAMETERS, {err_msg: 'canzhangs must be an array.'})
      return
    end
