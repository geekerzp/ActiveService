...
module KapaiHelper
  ...
  def create_gongfu
    ...
    if gongfu.nil?
      return render_result(ResultCode::ERROR, {err_msg: URI.encode("create gongfu error")})
    end
    unless gongfu.save
  ...
  def create_disciple
    ...
    if disciple.nil?
      return render_result(ResultCode::ERROR, {err_msg: URI.encode("create disciple error")})
    end
